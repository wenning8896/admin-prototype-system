...
import { shipToSeedRecords } from "../mocks/shipTo.mock";

const STORAGE_KEY = "csl-order-admin-ship-to-list";

export type ShipToFilters = {
  businessUnit?: string;
  region?: string;
  cg?: string;
  dealerCode?: string;
...
function readStoredRecords() {
  ...
  try {
    return JSON.parse(raw) as ShipToRecord[];
  } catch {
...
function persistStoredRecords(records: ShipToRecord[]) {
  if (typeof window === "undefined") {
    return;
  }
...
function getMergedRecords() {
  ...
  const merged = [...shipToSeedRecords];
...
function persistMergedRecord(nextRecord: ShipToRecord) {
  const stored = readStoredRecords();
  ...
  const seedIndex = shipToSeedRecords.findIndex((item) => item.id === nextRecord.id);
  if (seedIndex >= 0) {
...
export async function listShipToRecords(filters: ShipToFilters = {}) {
  ...
  const inheritedFromShipTo = filters.inheritedFromShipTo?.trim().toLowerCase();
  ...
  return getMergedRecords().filter((item) => {
    ...
    const matchesInherited = !inheritedFromShipTo || item.inheritedFromShipTo.toLowerCase().includes(inheritedFromShipTo);
...
export function exportShipToRecords(records: ShipToRecord[]) {
  const worksheet = utils.json_to_sheet(
    records.map((item) => ({
      ...
      "基础配置继承自（ShipTo）": item.inheritedFromShipTo,
      状态: item.status,
      创建时间: item.createdAt,
    })),
  );
  ...
  utils.book_append_sheet(workbook, worksheet, "ShipTo列表");
  writeFileXLSX(workbook, `ShipTo列表_${dayjs().format("YYYYMMDD_HHmmss")}.xlsx`);
}
...
function downloadTemplate(fileName: string, sampleStatus: "启用" | "停用") {
  const templateSheet = utils.json_to_sheet([
    {
      ...
      经销商名称: "辽宁嘉丰进出口贸易有限公司",
      ShipTo编码: "SHIPTO0001",
      "基础配置继承自（ShipTo）": "BASE-SHIPTO-001",
      状态: sampleStatus,
    },
  ]);
  templateSheet["!cols"] = [
  ...
  const instructionSheet = utils.aoa_to_sheet([
    ...
    ["基础配置继承自（ShipTo）", "是", "填写基础配置继承来源的 ShipTo 编码"],
    ["状态", "否", "导入新增默认启用；导入停用默认停用"],
  ]);
  instructionSheet["!cols"] = [{ wch: 24 }, { wch: 12 }, { wch: 46 }];
...
export function downloadShipToCreateTemplate() {
  downloadTemplate("ShipTo导入新增模板.xlsx", "启用");
}

export function downloadShipToDisableTemplate() {
  downloadTemplate("ShipTo导入停用模板.xlsx", "停用");
}

export async function importShipToRecords(file: File, mode: "create" | "disable") {
  ...
  rows.forEach((row, index) => {
    ...
    const inheritedFromShipTo = String(row["基础配置继承自（ShipTo）"] ?? "").trim();

    if (!businessUnit || !region || !cg || !dealerCode || !dealerName || !shipToCode || !inheritedFromShipTo) {
      skippedCount += 1;
      return;
    }

    const existing = merged.find((item) => item.shipToCode === shipToCode && item.businessUnit === businessUnit);
    const now = dayjs().format("YYYY-MM-DD HH:mm");

    persistMergedRecord({
      id: existing?.id ?? `shipto-import-${Date.now()}-${index}`,
      businessUnit,
      region,
      cg,
      dealerCode,
