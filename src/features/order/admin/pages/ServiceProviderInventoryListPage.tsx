...
import {
  exportServiceProviderInventory,
  listServiceProviderInventory,
  type ServiceProviderInventoryFilters,
} from "../services/serviceProvider.mock-service";

export function ServiceProviderInventoryListPage() {
  const [form] = Form.useForm<ServiceProviderInventoryFilters>();
  const { message } = App.useApp();
  const [loading, setLoading] = useState(true);
  const [items, setItems] = useState<ServiceProviderInventoryRecord[]>([]);

  async function loadData(filters: ServiceProviderInventoryFilters = {}) {
    setLoading(true);
    try {
      setItems(await listServiceProviderInventory(filters));
    } finally {
      setLoading(false);
    }
  }
  ...
  const columns: ColumnsType<ServiceProviderInventoryRecord> = [
  ...
  function handleExport() {
    exportServiceProviderInventory(items);
    void message.success("服务商库存列表已导出为 .xlsx 文件。");
  }
